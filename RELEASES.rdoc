...
* vSphere cloud
  ...
  * NICs are now renamed on Windows to correspond with the network name.
...
* Linux packages have been reorgnized. Source packages are now available for the RightLink and the RightLink sandbox from all RightLink repositories. The RightLink package now contains all RightLink code and gems, while the sandbox contains only Ruby and Rubygems.
* RightLink decomission bundle is now run on a separate thread.  
* vSphere: The floppy drive should no longer be automounted at boot. vSphere metadata is no transmitted via VMware extra config. VMware tools must be pre-installed on the image. When uploading images via ovftool make sure allowExtraConfig option is enabled as well.
* RightScript parameters that are changed from a value to unset will no longer remember the old value on subsequent runs. The value will now be set to a null value.
* RightScript credentials are now also masked in system logs and not just audit entries. These values will now show up as <hidden input INPUTNAME> instead of their value.
* Support for Ubuntu Raring (13.04) has been dropped.

== Bug fixes
* Fix for RightScript and Chef script blocks returning malformed characters. These malformed characters could raise encoding exceptions and cause audit entries to be lost. Characters not appearing to be UTF-8 encoded are now replaced with "?".
* Removed spurious RbConfig warnings during running of RightLink command line tools on windows.
* Ubuntu/Debian: The "rightlink" package now depends on lsb-release instead of lsb, reducing the list of dependent packages.
* vSphere cloud
  * SSH key no longer missing if an invalid network config is applied.
  * Removed startup delays due to waiting for DHCP to timeout if no DHCP network was specified.
  * Removed need to clean network configuration when snapshotting instances. RightLink will always reconfigure the network correctly at boot.
* Ubuntu: RightLink places all its managed repo config files in /etc/apt/sources.list.d. It now creates an empty /etc/apt/sources.list as well instead of simply removing it. This avoid compatibility issues with programs such update-notifier, which expects a file to exist at that location.
* RightLink no longer blocks console login when failing to enroll.

== Known limitations
* GCE: rs_reboot utility not currently work (as of Aug 13 2014). Reboot via the RightScale API is not currently supported for this cloud. Use system reboot instead (init 6 or shutdown -r).
...
* vSphere cloud
  * Remove /etc/udev/rules.d/70-persistent-net.rules before snapshotting instances on vSphere or else RightLink may have issues configuring networks on boot.
