...
* vSphere cloud
  ...
  * rs_ipN_name metadata key to provide name for corresponding NIC on Windows

= 6.0.5 (RightLink 6.0 release candidate 6)

Released 2014-05-14

== Changes to Existing Functionality

* Reverted to previous eventmachine gem

= 6.0.4 (RightLink 6.0 release candidate 5)

Released 2014-05-13

== New Features

* Added support to allow NTP server used for initial time sync to be configured by the RightScale Cloud Appliance for vSphere.

= 6.0.3 (RightLink 6.0 release candidate 4)
...
* Linux: Setting a static network configuration now disables DHCP client to avoid reconfiguration conflicts.
...
* Internal API client switched to non-blocking version.  This fixes issues with lost log messages and spurious error messages after running an rs_shutdown request
* Remove extraneous output when RightScripts exited with a non-success status
* System and chef rubygems now configured to use rightscale mirror
* Fix for rs_state sometimes returning an empty value on decommission
* /etc/apt/sources.list during repository configuration
