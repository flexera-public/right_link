...
* vSphere support
* rs_ohai in addition to original ohai support following clouds:
  vSphere
  CloudStack
  SoftLayer
* rs_config utility allows you to specify and get status of the followin features:
  decommisioning timeout in seconds
...
* The following metadata key/values will configure:
  RS_NTP : FQDN address of NTP server
  RS_BREAKPOINT : allows to stop RightLink service startup in cloud_init step. Usefull for debugging.
