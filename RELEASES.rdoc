...
* vSphere support
* rs_ohai in addition to original ohai supports following clouds:
  * vSphere
  * CloudStack
  * SoftLayer
* rs_config utility allows you to specify and get status of the followin features:
  * decommision timeout in seconds
  * managed login feature on/off
  * MOTD update on/off
  * Package repository freeze on/off
...
* metadata:rs_breakpoint=init_cloud_state allows to stop RightLink service startup right after init_cloud_state step. Usefull for debugging.
* Forced one time NTP time sync. Comma or space separated list of NTP servers can be set by:
  * setting RS_ntp value directly in metadata
  * via server tag rs_agent:ntp_servers
  * cloud might also set RS_ntp value in metadata(if this value is also set in user-data,
    the userdata value will win)

== Changes to Existing Functionality

* chef version have been updated to 11.6.0
* ruby version have been updated to 1.9.3 p484
* rs_ohai now behavir the same way as vanilla ohai
* AMQP has been replaced with HTTP in communition between RightLink and RightNet

== Known Limitation

* rs_ohai [cloudstack|softlayer|vsphere] provides limited attribute list.
* vSphere cloud
  * Only supported in RightLink v6.0+
  ...
  * For RightLink to work on a VMware Linux template (aka image), the operating system must
    automount the fd0 device at /mnt/metadata.  This can be done by adding the following line to /etc/fstab.
