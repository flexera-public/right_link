= 6.0.X (RightLink 6.0 release candidate 2)
...
* Original vanilla ohai plugin doesn't support cloud and cloud specific attributes under Windows platform. Use RightScale rs_ohai plugin instead.
* There are DHCP timeout delays on raw vSphere image on Linux platform during booting; to avoid it rightboot has to be launched before network configuration. 
* Blank sources list (/etc/apt/sources.list) is absent by default on Ubuntu. It could cause various problems over time. 
* Warning message about uninitialized constant is printed to stdout when RightScript or Chef recipe is running on Windows platform. 
* Message that cookie-file is absent could be printed as feedback on RightLink CLI tool running if rightlink service is not started. 
* Windows: current log level is printed to stdout on Windows platform during executing of some RightLink CLI tools.
* There is no possibility to SSH into the instance if RightLink hasn't been enrolled successfully. 
...
* rs_config utility command line utility added.  This utility allows you to configure RightLink options. Of note is the ability to set the default timeout to wait on decomission scripts before forcing reboot or termination. Default value is 180 seconds.
...
* Firewall friendliness
  * AMQP has been replaced with HTTP in communition between RightLink agent and RightScale core site.
  * Syncing with NTP, RightLink patching, and software repo mirrors will now go through loadbalancer specified in RS_ISLAND userdata variable set by RightScale when the instance launches.
...
* Removed custom Chef resources: right_link_tag, remote_recipe, server_collection and rs_shutdown. Use the RightLink command-line utilties instead.

== Known Limitations

* Initiating a system reboot from a script running inside the boot bundle may lead to unexpected error messages. Any decommission script will block behind the boot bundle until its complete, which may generate a timeout errors from the managin RightLink agent process.
