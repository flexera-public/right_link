= 6.1.0 (RightLink 6.0 RC1)

Released 2014-08-11

notes:
  * Rebundled images will now always reconfigure networking correctly at startup, even if network config is not cleaned out as advised in 6.0.6
* azure metadata - refactored internally


== New Features
* vSphere cloud
  * Multiple network interfaces are now supported.
  * Start/stop is now supported.
  * NICs are now renamed on Windows to correspond with the network name.
* Support for Ubuntu Trusty (14.04)
* Support for CentOS/RedHat Enterprise Linux 7
