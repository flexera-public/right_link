...
*Note:* Before installing the RightLink package be sure that time of the server is synchronized with the time clocks over a network. Otherwise, you can use Network Time Protocol (NTP) for synchronizing.
...
You can now bundle your image and boot RightScale server with it.
...
*Note:* Before installing the RightLink package be sure that time of the server is synchronized with the time clocks over a network.
