...
As of May 2014, the latest stable release of RightLink is v6.0. 
...
\* These variants are not officially supported and validated to work with RightLink 6.0.  Full support should be available with RightLink 6.1.

To learn how to install stable RightLink, refer to our support site:

http://support.rightscale.com/12-Guides/RightLink/RightLink_6.0/Install_RightLink


== Beta Packages - RightLink v6.1
...
Create a yum source:
  ...
  [rightlink]
  name=RightLink
  baseurl=http://rightlink-beta.test.rightscale.com/yum/1/el/6/$basearch/
  gpgcheck=1
...
and release code name. For instance, on an amd64 system that is running:
  ...
  # Ubuntu 13.04 (raring)
  cat > /etc/apt/sources.list.d/rightlink.sources.list <<EOF
  deb [arch=amd64] http://rightlink-beta.test.rightscale.com/apt raring main
  EOF
