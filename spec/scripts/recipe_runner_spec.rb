...
require File.expand_path(File.join(File.dirname(__FILE__), '..', '..', 'scripts', 'recipe_runner'))

module RightScale
  describe RecipeRunner do
    def send_command(args, forwarder_opts,timeout=BundleRunner::DEFAULT_TIMEOUT, verbose=false)
    ...
    context 'rs_run_recipe --version' do
      it 'reports RightLink version from gemspec' do
        run_bundle_runner('--version')
        @output.join('\n').should match /rs_run_recipe \d+\.\d+\.?\d* - RightLink's Chef recipes Runner \(c\) \d+-\d+ RightScale Inc/
      end
    end

    context 'rs_run_recipe --help' do
      it 'should show usage info' do
        usage = Usage.scan(File.expand_path(File.join(File.dirname(__FILE__), '..', '..', 'scripts', 'recipe_runner.rb')))
    ...
    context 'rs_run_recipe -n nginx -j attribs.js' do
      ...
      it 'should run recipe \'nginx\' using given JSON attributes file' do
        send_command(['-n', 'nginx', '-j', json_file], {
          :recipe => 'nginx',
          :json => expected_value
        })
      end
    ...
    context 'rs_run_recipe -i 14 -r "tag1 tag2" -s single' do
      it 'should run recipe with id 14 on a single server with tags: tag1, tag2' do
        send_command(['-i', '14', '-r', 'tag1 tag2', '-s', 'single'], {
          :tags => ["tag1", "tag2"],
          :recipe_id => "14",
          :selector => :any,
          :json => nil
    ...
    context 'rs_run_recipe -i 14 --policy SWAG' do
      it 'should run recipe with id 14 auditing on SWAG policy' do
        send_command('-i 14 --policy SWAG'.split, {
          :recipe_id => "14",
          :json => nil,
          :policy => "SWAG"
        })
    ...
    context 'rs_run_recipe -i 14 --thread thread123' do
      it 'should run recipe with id 14 on thread123 thread' do
        send_command('-i 14 --thread thread123'.split, {
          :recipe_id => "14",
          :json => nil,
          :thread => "thread123"
        })
    ...
    context 'rs_run_recipe -i 14 -c /etc/etc' do
      it 'should change set directory containing configuration for all agents to /etc/etc' do
        ...
        send_command('-i 14 -c /etc/etc'.split, {
          :recipe_id => "14",
          :json => nil
        })
      end
