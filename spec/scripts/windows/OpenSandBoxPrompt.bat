...
if "%1" equ "dev" (
  if exist "%~dps0..\..\..\..\right_link" set RS_SANDBOX_HOME=%~dps0..\..\..\..
)
...
:AfterRuby
if exist "%RS_SANDBOX_HOME%\right_link_package\instance\bin\windows" (
  set RS_SANDBOX_BIN_WINDOWS=%RS_SANDBOX_HOME%\right_link_package\instance\bin\windows
) else (
  set RS_SANDBOX_BIN_WINDOWS=%RS_SANDBOX_HOME%\bin\windows
)
set PATH=%PATH%;%RS_SANDBOX_BIN_WINDOWS%;%RS_SANDBOX_HOME%\right_link\scripts\windows


rem # include spec scripts on path
set PATH=%PATH%;%~dps0.
start "RightScale SandBox" cmd.exe /k "cd /d %RS_SANDBOX_HOME%"
